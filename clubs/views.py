...
def create_club(request):
    ...
    # Validate name and description (alphanumeric and spaces only)
    name_validator = RegexValidator(r'^[a-zA-Z0-9 ]+$', 'Club name must contain only letters, numbers, and spaces.')
    description_validator = RegexValidator(r'^[a-zA-Z0-9 .,!?]+$', 'Club description contains invalid characters.')
    try:
        name_validator(name)
...
def add_club_role(request, club_id):
    ...
    try:
        ...
        role_type = request.data.get('type', '').strip()
        ...
        if not isinstance(members, list) or not members:
            return Response({'error': 'At least one member is required, and it must be a list.'}, status=400)

        # Validate role_name (alphanumeric and spaces only)
        role_name_validator = RegexValidator(r'^[a-zA-Z0-9 ]+$', 'Role name must contain only letters, numbers, and spaces.')
        try:
            role_name_validator(role_name)
        except ValidationError as e:
            return Response({'error': str(e)}, status=400)

        # Validate role_type (must be 'standard' or 'custom')
        if role_type not in ['standard', 'custom']:
            return Response({'error': 'Role type must be either "standard" or "custom".'}, status=400)

        # Validate member IDs (must be integers)
        for member_id in members:
            if not isinstance(member_id, int):
                return Response({'error': f'Invalid member ID: {member_id}. Must be an integer.'}, status=400)
