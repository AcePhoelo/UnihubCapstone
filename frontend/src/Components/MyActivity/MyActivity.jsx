    ...
    useEffect(() => {
        const fetchProfileAndActivity = async () => {
            ...
            const fetchUserActivity = async () => {
                try {
                    const clubsResponse = await fetch(`https://curtinunihubplus.com/profile/students/${profile.studentid}`, {
                        headers: {
                            'Authorization': `Bearer ${token}`,
                            'Content-Type': 'application/json',
                        },
                    ...
                    if (clubsResponse.status === 401 || clubsResponse.status === 403) {
                        ...
                        if (refreshToken) {
                            const refreshResponse = await fetch('https://curtinunihubplus.com/api/token/refresh/', {
                                method: 'POST',
                                headers: {
                                    'Content-Type': 'application/json',
                                },
                            ...
                            if (refreshResponse.ok) {
                                ...
                                const newResponse = await fetch(`https://curtinunihubplus.com/profile/students/${profile.studentid}`, {
                                    headers: {
            ...
            const processUserActivity = (userData) => {
                ...
                const leaderClubs = Array.isArray(userData.leadership_clubs)
                    ? userData.leadership_clubs.map(club => ({
                        ...
                        logo: club.logo ? (club.logo.startsWith('http') ? club.logo : `https://curtinunihubplus.com${club.logo}`) : null,
                        banner: club.banner ? (club.banner.startsWith('http') ? club.banner : `https://curtinunihubplus.com${club.banner}`) : null,
                        imageUrl: club.banner ? (club.banner.startsWith('http') ? club.banner : `https://curtinunihubplus.com${club.banner}`) : null,
                        logoUrl: club.logo ? (club.logo.startsWith('http') ? club.logo : `https://curtinunihubplus.com${club.logo}`) : null
                    }))
                    : [];
        ...
        const fetchUserEvents = async () => {
            try {
                ...
                const registrationsResponse = await fetch(
                    `https://curtinunihubplus.com/api/event/event_registration/?user_id=${profile.studentid}`, 
                    {
                        headers: {
                            'Authorization': `Bearer ${token}`,
                            'Content-Type': 'application/json',
                ...
                if (registrationsData.results && registrationsData.results.length > 0) {
                    ...
                    if (imageUrl && !imageUrl.startsWith('http')) {
                        imageUrl = `https://curtinunihubplus.com${imageUrl}`;
                    }
                        return {
                            id: event.id,
