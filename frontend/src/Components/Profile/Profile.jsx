...
const Profile = () => {
            ...
            const fetchProfileData = async () => {
                ...
                try {
                    ...
                    const endpoint = isViewingOwnProfile
                        ? 'https://54.169.81.75:8000/profile/profile/' 
                        : `https://54.169.81.75:8000/profile/students/${requestedStudentId}/`;
                        ...
                        const processClubImages = (clubs) => {
                            return clubs.map(club => ({
                                ...
                                logo: club.logo ? (club.logo.startsWith('http') ? club.logo : 
                                      `https://54.169.81.75:8000${club.logo}`) : ''
                            }));
                        };
    ...
    const handleProfilePicChange = async (e) => {
        ...
        try {
            ...
            const accessToken = localStorage.getItem('access_token');
            const response = await fetch('https://54.169.81.75:8000/profile/update-picture/', {
                method: 'PUT',
                headers: {
                    'Authorization': `Bearer ${accessToken}`,
                },
