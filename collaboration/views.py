...
def send_email(request):
    if request.method == 'POST':
        try:
            ...
            your_club = data.get('your_club', '').strip()
            sender_email = data.get('name', '').strip()
            ...
            try:
                ...
                return JsonResponse({'error': f'Invalid email address: {str(e)}'}, status=400)

            # Validate club name (alphanumeric and spaces only)
            if not your_club.replace(' ', '').isalnum():
                return JsonResponse({'error': 'Club name must contain only letters, numbers, and spaces.'}, status=400)

            # Validate content length
            if len(content) > 1000:
                return JsonResponse({'error': 'Email content must not exceed 1000 characters.'}, status=400)
